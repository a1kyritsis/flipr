...
def stock_prediction_model(
    ...
    if individual == False:
        ...
        for i in indices:
            ...
            for x in data_test.index:
                if x in new_test.index and trained == True:
                    lis.append(signals[count].item())
                    count += 1
                else:
                    lis.append(np.NaN)
            new = pd.DataFrame(lis, index=data_test.index)
            if type(dataframe) is list:
                dataframe = new.T
            else:
                dataframe = pd.concat([dataframe, new.T])
    ...
    #Holds the prediction for every company on every day
    #0 is short, 1 is long, 2 is hold, nan means there was not sufficient data to make a prediction
    #so ignore the nans
    df_ready_for_portfolio = dataframe.reset_index(drop=True)
    ...
    if print_results == True:
        ...
        return 0, sum(chk)/len(chk), max(chk), min(chk)